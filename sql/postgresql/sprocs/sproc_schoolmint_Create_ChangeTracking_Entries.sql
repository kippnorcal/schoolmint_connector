CREATE PROCEDURE custom."sproc_schoolmint_Create_ChangeTracking_Entries"()
LANGUAGE plpgsql
AS $$
BEGIN
/***************************************************************************
...
SELECT  
   ...
   , CASE 
      WHEN raw1."Application_ID" <> COALESCE(back1."Application_ID", 999999) THEN 'NEW'
      WHEN raw1."Application_Status" = back1."Application_Status" THEN 'NonStatusChange'
      WHEN raw1."Application_Status" = COALESCE(back1."Application_Status", raw1."Application_Status") THEN 'StatusUpdate'
      WHEN raw1."Application_Status" <> COALESCE(back1."Application_Status", 999) THEN 'StatusChange'
     END
   , lk."SchoolYear4Digit_int"
FROM custom."schoolmint_ApplicationData_raw" raw1
LEFT JOIN custom."schoolmint_ApplicationData_raw_backup" back1
...
AND NOT EXISTS (
    SELECT 1
    FROM custom."schoolmint_ApplicationData_changehistory" hist
    WHERE hist."Application_ID" = raw1."Application_ID"
      AND hist."Last_Status_Change" = raw1."Last_Status_Change"
    );
